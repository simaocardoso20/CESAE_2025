...
INNER JOIN Venda ON Cliente.ID = Venda.ClienteID
INNER JOIN Funcionario ON Funcionario.ID = Venda.FuncionarioID
...
-- ex-o.4
SELECT Cliente.Nome AS NomeCliente, Produto.Nome AS NomeProduto, Produtovenda.Quantidade, Produtovenda.Quantidade * Produto.Preco AS Subtotal, Produto.Preco AS PrecoUnitario FROM Venda
JOIN Cliente ON Venda.ClienteID = Cliente.ID
JOIN Produtovenda ON Venda.ID = Produtovenda.VendaID
JOIN Produto ON Produtovenda.ProdutoID = Produto.ID
WHERE Produtovenda.Quantidade * Produto.Preco > 300 AND Produtovenda.Quantidade > 1 AND Venda.DataVenda > '2023-04-01'
